...
// AI 服務提供商與模型
const PROVIDERS = [
  { id: "gemini", name: "Google Gemini", model: "gemini-2.5-flash-lite" },
  { id: "nvidia", name: "NVIDIA Nemotron", model: "nvidia/nemotron-mini-4b-instruct" }
];
          ...
          <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
            ...
            <div className="flex flex-wrap gap-2.5 shrink-0">
              <div className="text-[11px] font-mono bg-white px-2.5 py-1.5 border border-slate-200 rounded-lg text-slate-600 font-semibold leading-none shadow-3xs">
                Model: {aiProvider === "nvidia" ? "nemotron-mini-4b-instruct" : "gemini-2.5-flash-lite"}
              </div>
              <div className="text-[11px] font-mono bg-white px-2.5 py-1.5 border border-slate-200 rounded-lg text-slate-600 font-semibold leading-none shadow-3xs">
                  ...
                  {PROVIDERS.map((prov) => (
                    <button
                      ...
                      className={`text-left p-3 rounded-xl border transition-all duration-150 relative cursor-pointer ${
                        aiProvider === prov.id
                          ? "bg-indigo-50/80 border-indigo-200 text-indigo-700 ring-4 ring-indigo-500/5 shadow-2xs"
                          : "bg-white border-slate-200 text-slate-500 hover:bg-slate-50/80 hover:border-slate-350"
                      }`}
                    >
                      <div className="font-bold text-xs flex items-center justify-between">
                        <span className={aiProvider === prov.id ? "text-slate-900 font-extrabold" : "text-slate-700"}>{prov.name}</span>
                        {aiProvider === prov.id && <span className="w-1.5 h-1.5 rounded-full bg-indigo-600"></span>}
                      </div>
                    ...
                    >
                      <div className="space-y-1 min-w-0 pr-2">
                        ...
                        <div className="flex items-center space-x-2 text-[10px]">
                          ...
                          <span className="bg-slate-100 text-slate-650 px-1.5 py-0.5 rounded uppercase font-bold text-[9px] border border-slate-200">
                    ...
                    <div className="flex flex-col items-center justify-center space-y-4">
                      ...
                      <div className="space-y-1 block text-center">
                        ...
                        <p className="text-xs text-slate-400">
                          本階段由 {aiProvider === "nvidia" ? "NVIDIA Nemotron" : "Google Gemini"} 核心進行高階譯析及多語翻譯同步整合
                        </p>
                      </div>
                    </div>
                    {/* 骨架模擬條 */}
      ...
      <footer id="app_footer" className="border-t border-slate-200 bg-white py-6 mt-12 text-xs text-slate-500 shadow-xs">
        <div className="max-w-7xl mx-auto px-4 text-center space-y-2">
          ...
          <p className="font-mono text-[10px] text-slate-400">
            基於 Vercel Serverless Function 微服務與 Google Gemini / NVIDIA 頂級 AI 模型架構構建
          </p>
        </div>
      </footer>
